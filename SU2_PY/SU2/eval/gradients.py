...
def adjoint(func_name, config, state=None):
    ...
    with redirect_folder(ADJ_NAME, pull, link) as push:
        with redirect_output(log_adjoint):
            ...
            if multi_objective:
                config["OBJECTIVE_FUNCTION"] = ", ".join(
                    func_name
                )  # Can this be deleted?
...
def stability(func_name, config, state=None, step=1e-2):
    ...
    grads = [(g_1 - g_0) / step for g_1, g_0 in zip(grads_1, grads_0)]
...
def multipoint(func_name, config, state=None, step=1e-2):
    ...
    for i in range(len(weight_list) - 1):
        ...
        if (
            MULTIPOINT_ADJ_NAME in state.FILES
            and state.FILES[MULTIPOINT_ADJ_NAME][i + 1]
        ):
        ...
        if "MULTIPOINT_MESH_FILENAME" in ztate.FILES:
            if "deform" in ztate.FILES.MESH:
                ztate.FILES.MESH = su2io.add_suffix(
                    ztate.FILES.MULTIPOINT_MESH_FILENAME[i + 1], "deform"
                )
                konfig.MESH_FILENAME = su2io.add_suffix(
                    ztate.FILES.MULTIPOINT_MESH_FILENAME[i + 1], "deform"
                )
            else:
    ...
    for variable in range(len(grads[0])):
        ...
        for point in range(len(weight_list)):
            grad[variable] = (
                grad[variable] + float(weight_list[point]) * grads[point][variable]
            )
